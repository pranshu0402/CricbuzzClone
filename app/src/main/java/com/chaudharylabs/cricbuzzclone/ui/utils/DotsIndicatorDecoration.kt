...
class DotsIndicatorDecoration(active: String, inactive: String) : RecyclerView.ItemDecoration() {
    private val colorActive = Color.parseColor(active)
    private val colorInactive = Color.parseColor(inactive)
    ...
    private fun drawInactiveIndicators(
        ...
        for (i in 0 until itemCount) {
            c.drawCircle(start, indicatorPosY, 4F, mPaint)
            start += itemWidth
        }
    }
    ...
    private fun drawHighlights(
        ...
        if (progress == 0f) {
            ...
            c.drawCircle(highlightStart, indicatorPosY, 4F, mPaint)
        } else {
            val highlightStart = indicatorStartX + itemWidth * highlightPosition
